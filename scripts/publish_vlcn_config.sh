...
DEFAULT_VLCN_ROOT="$REPO_ROOT/3rd-party/js"

if [[ -z "${VLCN_ROOT:-}" && -d "$DEFAULT_VLCN_ROOT" ]]; then
	VLCN_ROOT="$DEFAULT_VLCN_ROOT"
elif [[ -z "${VLCN_ROOT:-}" && -d "$SCRIPT_DIR/../3rd-party/js" ]]; then
	VLCN_ROOT="$SCRIPT_DIR/../3rd-party/js"
elif [[ -z "${VLCN_ROOT:-}" ]]; then
	VLCN_ROOT="$DEFAULT_VLCN_ROOT"
fi
