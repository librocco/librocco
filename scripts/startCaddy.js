...
*/
let errors = [];
if (!process.env.CADDY_EMAIL)
  errors.push("CADDY_EMAIL should be defined and be a valid email address");
if (!process.env.CF_ZONE_ID)
  errors.push("CF_ZONE_ID should be defined and be a Cloudflare DNS zone ID");
if (!process.env.CLOUDFLARE_API_TOKEN)
  errors.push("CLOUDFLARE_API_TOKEN should be defined and valid");
if (!fs.existsSync(CADDY_DIR))
  errors.push(`CADDY_DIR directory (${CADDY_DIR}) does not exist`);
...
async function createCFRecords(localIP) {
  ...
  for (const { name, content } of records) {
    ...
    if (resultJson.errors.length > 0) {
      if (
        resultJson.errors.length !== 1 ||
        resultJson.errors[0].code !== 81058
      ) {
        console.error(
          `\x1b[31m\x1b[1mError creating DNS record:\x1b[0m`,
          resultJson.errors,
        );
        process.exit(1);
      }
...
(async function main() {
  ...
  if (!checkCaddyCapabilities()) {
    console.error(
      `\x1b[31m\x1b[1mCaddy does not have the required capabilities. Please run:\x1b[0m`,
    );
