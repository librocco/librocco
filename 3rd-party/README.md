...
Normal dependency delivery is now registry-backed (`npm.codemyriad.io`).  
The workflows below only exist to support:

- local source-mode validation against unpublished `@vlcn.io/*` changes
- legacy R2 artefact workflows that are still used by some existing CI jobs

For normal work, use the registry path and the published snapshot versions declared in package manifests and `globalOverrides`.
