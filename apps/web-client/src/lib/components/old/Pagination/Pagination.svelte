...
{:else}
	<nav {...$$restProps} class="flex {$$props.class}">
		<button
			class="button-inactive button {value === 0 ? 'button-disabled' : 'button-hover'}"
			disabled={value === 0}
		...
		{#each itemsToRender as item}
			{#if item === null}
				<button disabled class="button-inactive button">...</button>
			{:else}
				<button
					disabled={value === item}
					class="button-hover button {value === item ? 'button-active' : 'button-inactive'}"
		...
		<button
			class="button-inactive button {value === itemsToRender[itemsToRender.length - 1] ? 'button-disabled' : 'button-hover'}"
