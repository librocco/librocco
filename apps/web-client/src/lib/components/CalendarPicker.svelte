...
<div>
	<div class="border-base-100 bg-base-100 px mt-1.5 flex min-w-[200px] items-center gap-x-2 border p-1.5" use:melt={$field}>
		...
		<button
			...
			data-open={$open}
			class="btn btn-neutral p-1 transition-all"
			use:melt={$trigger}
		>
			<Calendar size={20} />
		</button>
	...
	>
		<div class="bg-base-100 w-full rounded-lg p-3 shadow-sm" use:melt={$calendar}>
			<div data-testid={testId("calendar-picker-month-select")} class="flex items-center justify-between pb-2">
				<button class="btn btn-neutral p-1 transition-all" use:melt={$prevButton}>
					<ChevronLeft size={24} />
				</button>
				<div class="flex items-center gap-6" use:melt={$heading}>
					{$monthHeading}
				</div>
				<button class="btn btn-neutral p-1 transition-all" use:melt={$nextButton}>
