<script lang="ts">
	...
	import { createRadioGroup, createCollapsible, melt } from "@melt-ui/svelte";
	...
	// Underdelivery behaviour
	export let underdeliveryBehaviourStore: Writable<"pending" | "queue"> = writable("pending");
	const {
		elements: { root: radioRoot, item },
		helpers: { isChecked }
	} = createRadioGroup({
		value: underdeliveryBehaviourStore
	});

	const underdeliveryOptions = ["pending", "queue"];
</script>

<div use:melt={$collapsibleRoot} class="rounded-lg border border-neutral-200">
	<div use:melt={$trigger} class="bg-card cursor-pointer px-4 py-3 transition-all hover:bg-neutral-100">
		<div class="flex items-center justify-between gap-4">
			<div class="flex min-w-0 flex-1 items-start gap-3">
				...
				<button class="rounded p-1 transition-colors hover:bg-neutral-200" on:click={(e) => e.stopPropagation()}>
					<ChevronDown
				...
				<Table variant="naked" columnWidths={["2", "3", "4", "2", "2", "2"]}>
					...
					<svelte:fragment slot="rows">
						{#each books as book (book.isbn)}
							...
							<TableRow
								variant="naked"
								className="hover:bg-neutral-50 transition-all duration-100 {isLastBook ? '' : 'border-b border-neutral-100'}"
							>
