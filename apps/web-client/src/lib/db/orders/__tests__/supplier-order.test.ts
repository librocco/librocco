...
describe("Suppliers order creation", () => {
	...
	it("sees possible supplier orders from client orders", async () => {
		...
		expect(await getPossibleSupplerOrderLines(db)).toStrictEqual([
			{ supplier_id: 1, quantity: 1, isbn: "1", supplier_name: "Science Books LTD" },
			{ supplier_id: 2, quantity: 1, isbn: "2", supplier_name: "Phantasy Books LTD" }, // This is now from supplier 2
			{ supplier_id: 2, quantity: 2, isbn: "3", supplier_name: "Phantasy Books LTD" }
		]);
	});
