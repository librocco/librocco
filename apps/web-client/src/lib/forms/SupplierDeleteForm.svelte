<script lang="ts">
	...
	import { supplierDeleteSchema, type SupplierDeleteFormSchema } from "$lib/forms/schemas";
	...
	const matchConfirmation = displayName
		...
		.replace(/^_+|_+$/g, "");
	const schema = supplierDeleteSchema(matchConfirmation);

	export let options: FormOptions<SupplierDeleteFormSchema>;
...
<form use:enhance method="POST" class="flex max-w-lg flex-col gap-y-6">
	<div class="flex flex-col justify-between gap-y-6 p-4">
		<label class="form-control basis-1/2" id="year-field-container">
			<div class="label">
				<span class="label-text">{$LL.forms.supplier_delete.labels.confirm_typing()}</span>
			</div>
			<input
			...
			/>
			<div class="label">
				<span class="label-text-alt">{$LL.forms.supplier_delete.labels.type_instruction({ matchConfirmation })}</span>
			</div>
		</label>
	</div>
	...
	<div class="flex w-full justify-end gap-x-2 p-4">
		<button class="btn-secondary btn-outline btn" on:click={onCancel} type="button"
			>{$LL.forms.supplier_delete.labels.cancel_button()}</button
		>
		<button class="btn-error btn" type="submit">{$LL.forms.supplier_delete.labels.confirm_button()}</button>
	</div>
</form>
