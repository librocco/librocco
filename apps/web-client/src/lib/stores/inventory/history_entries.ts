...
import type { CreateDisplayEntriesStore, Result } from "$lib/types/inventory";
import { observableFromStore, readableFromStream } from "$lib/utils/streams";
import { mapMergeBookWarehouseData } from "$lib/utils/misc";

export const createHistoryStore: CreateDisplayEntriesStore = (ctx, db, committedNotesListStream, warehouseListStream, dateValue) => {
