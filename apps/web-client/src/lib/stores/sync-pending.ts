...
/**
 * Attach monitoring for pending local changes for a database and provide a cleanup function.
 *
 * Initializes last-ack state for the given `dbid`, subscribes to `rx` events and (in browser environments) cross-tab storage events to keep pending-change counts and timestamps synchronized, performs an initial refresh of pending counts, and keeps internal state updated for the attached `db`.
 *
 * @param db - The database instance to monitor
 * @param rx - The application's reactive event source used to trigger refreshes
 * @param dbid - The identifier for the database instance (used for persisted keys and cross-tab synchronization)
 * @returns A function that detaches all listeners and clears the monitor state for the attached database
 */
...
/**
 * Stops monitoring pending changes and restores the tracker to its default state.
 *
 * Clears any active subscriptions, forgets the current database and database id, resets the last-acknowledged version and any in-flight refresh, and sets `pendingChangesCount` to 0 and `pendingChangesSince` / `pendingChangesLastActiveAt` to `null`.
 */
