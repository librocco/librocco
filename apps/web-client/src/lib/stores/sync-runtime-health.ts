...
/**
 * Update the sync runtime health store using a sync status payload.
 *
 * @param payload - Status information from the sync runtime. If `payload.ok` is true, updates the last handshake time and sets the last-ack time when `payload.ackDbVersion` is present; if `payload.ok` is false, records a new error entry using `payload.reason` and `payload.message`.
 */
...
/**
 * Record the current time as the most recent sync acknowledgement and persist the change.
 *
 * Updates the store's `lastAckAt` to the current timestamp and, when running in a browser,
 * attempts to write `{ lastStatusAt, lastAckAt }` to localStorage under the shared runtime key;
...
/**
 * Reset the sync runtime health store to its initial state.
 *
 * This sets all timestamps to `null` and clears the recent error history.
 */
