...
function getSchema(): [name: string, version: bigint, content: string] {
	...
	const fpath = path.resolve(process.cwd(), "src", "sync_server", "schemas", name);
...
async function getSchemaNameAndVersion(db: Database): Promise<[string, bigint] | null> {
	...
	const version = (await db.prepare("SELECT value FROM crsql_master WHERE key = 'schema_version'").pluck().safeIntegers().get()) as
		| bigint
		| undefined;
