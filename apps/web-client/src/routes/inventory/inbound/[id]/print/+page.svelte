...
<header>
  <h1>Printable Open Inbound Note</h1>
  {#if note}
...
<style>
  ...
  .note-details, .book-entries {
    margin-bottom: 20px;
  }
  .note-details h2, .book-entries h3 { /* Adjusted h2 to h3 for entries */
    font-size: 1.2em;
  ...
  @media print {
    ...
    @page {
      @top-center {
        content: "Inbound Note - " var(--note-name, "N/A");
        font-size: 10pt;
        color: #333;
      }
      @bottom-center {
