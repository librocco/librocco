<script lang="ts">
	...
	import { PopoverWrapper, Dialog, AsyncData, SkeletonTable, ConfirmDialog } from "$lib/components";
	...
	$: if (dbCtx) {
		disposer?.();
		const disposer1 = dbCtx.rx.onPoint("supplier", BigInt(supplier?.id || 0), () => invalidate("supplier:data"));
		const disposer2 = dbCtx.rx.onRange(["supplier_order", "supplier_order_lines"], () => invalidate("supplier:orders"));
		disposer = () => {
	...
	$: ({ supplier: supplierP, publishersData: publishersDataP, orders: ordersP, plugins, dbCtx: dbCtxP } = data);

	$: combined = Promise.all([dbCtxP, supplierP, publishersDataP, ordersP]);
	...
	let supplier: import("$lib/db/cr-sqlite/types").Supplier | null = null;
	let assignedPublishers: string[] = [];
	let publishersAssignedToOtherSuppliers: string[] = [];
	let publishersUnassignedToSuppliers: string[] = [];
	let orders: import("$lib/db/cr-sqlite/types").PlacedSupplierOrder[] = [];
...
<Page title={$LL.supplier_orders_page.title.supplier_orders()} view="orders/suppliers/id" {db} {plugins}>
	<svelte:fragment slot="main">
		<AsyncData data={combined} let:resolved>
			{@const [dbCtx, supplier, pData, orders] = resolved}
			{@const { assignedPublishers, publishersAssignedToOtherSuppliers, publishersUnassignedToSuppliers } = pData}
			{((db = dbCtx.db), "")}
			...
			<div class="flex h-full flex-col gap-y-4 max-md:overflow-y-auto md:flex-row md:divide-x">
				...
				<div class="mb-20 flex h-full w-full flex-col gap-y-6 md:overflow-y-auto">
									...
									<tbody>
										{#each publishersUnassignedToSuppliers as publisher}
											<tr class="hover focus-within:bg-base-200">
												...
												<td class="px-2 text-end"
													><button
														on:click={handleAssignPublisher(publisher)}
														class="btn-primary btn-xs btn flex-nowrap gap-x-2.5 rounded-lg">{t.labels.add_to_supplier()}</button
													></td
												>
											</tr>
										{/each}
					...
					<div class="h-full overflow-x-auto">
						<div class="h-full">
							<OrderedTable {orders} on:reconcile={handleReconcile} on:download={handleDownload} />
						</div>
					</div>
				</div>
			</div>
