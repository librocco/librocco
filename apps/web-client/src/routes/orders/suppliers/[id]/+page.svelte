<script lang="ts">
	...
	import { createLayoutDataSource } from "./dataLoad";
	...
	const layoutDataSource = createLayoutDataSource(rx, () => getDb(app), supplierId, layoutData ?? undefined);

	const supplier = $derived(layoutDataSource.data.supplier);
