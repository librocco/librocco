<script lang="ts">
	...
	import { dbNamePersisted, resetDB } from "$lib/db";

	import { SettingsForm, settingsSchema } from "$lib/forms";
	...
	const handleDelete = (name: string) => async () => {
		...
		if (!files.includes(get(dbNamePersisted))) {
			await handleSelect(files[0] || "dev")();
		}
	};
				...
				<div class="w-full basis-2/3">
					<ul data-testid={testId("database-management-list")} class="h-[240px] w-full overflow-y-auto overflow-x-hidden border">
						{#each files as file (file)}
							{@const active = file === $dbNamePersisted}
							{#if selection}
								<li
									data-active={active}
