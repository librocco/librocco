...
import { newTestRunner as newInventoryTestRunner } from "@test-runner/runner";
...
import * as inventory from "./inventory/tests";
...
type DbKind = keyof typeof tests;

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const newTestRunner = (kind: DbKind) => newInventoryTestRunner(testDataLoader);

Object.entries(tests).forEach(([dbKind, tests]) => {
	describe(`${dbKind} tests`, async () => {
		const runner = await newTestRunner(dbKind as DbKind);
