...
/**
  ...
  * @param {Number} progress - chunk size for incrementation
  */
let completed = 0;
function downloading(progress: ProgressBar, chunkSize: number) {
    if (completed <= progress.total!) {
        progress.render(completed += chunkSize);
    }
}
...
async function CSVToCouch({ filePath, columns, couchdbName = "books",
    couchdbURL = "http://admin:admin@localhost:5984", columnSeparator = ",", chunkSize = 1000, progress }: {
        filePath: string,
    ...
    try {
        ...
        for await (const chunk of inChunks(readCSV(f, options), chunkSize, columnsArr)) {
            await bulkDocs(chunk, couchdbName, couchdbURL)
            downloading(progress, chunkSize);
...
program.command("parse", "parses csv file into the db")
    ...
    .action(async () => {
        ...
        await CSVToCouch({
            ...
            columnSeparator: program.separator, chunkSize: program.chunkSize,
            progress: progress,
...
/**
  ...
  * @return {Iterator} - An iterator over the chunks
  */
const inChunks = async function*<T>(sequence: AsyncIterable<AsyncIterable<string>>, chunkSize: number, columnsArr: string[]) {
