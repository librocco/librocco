import { WLogger, defaultLogger } from "../worker/logger";
...
export default class WebSocketTransport implements Transport {
	...
	constructor(options: TransporOptions, private logger: WLogger = defaultLogger) {
	...
	#openSocketAndKeepAlive(options: TransporOptions) {
		...
		const socket = new WebSocket(options.url, [
			btoa(
				`${options.authToken != null ? `auth=${options.authToken},` : ""}room=${options.room
				}`
			).replaceAll("=", ""),
		]);
	...
	#processEvent = (data: Uint8Array) => {
		...
		this.logger.log("[web_socket_transport:process_event]", "tag:", msg._tag)
