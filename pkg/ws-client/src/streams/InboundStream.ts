...
import { DB } from "../DB";
import { WLogger } from "worker/logger";
...
export default class InboundStream {
	...
	constructor(db: DB, transport: Transport, private logger: WLogger = console) {
	...
	receiveChanges = async (msg: Changes) => {
		...
		if (msg.changes.length == 0) {
			this.logger.log("[inbound_stream]", "changes empty")
			return;
		}
		...
		try {
			this.logger.log("[inbound_stream]", "applying changes")
