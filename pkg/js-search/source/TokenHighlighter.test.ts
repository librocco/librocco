...
describe("TokenHighlighter", () => {
  ...
  it("should highlight tokens that equal the full string", () => {
    ...
    const tokens = ["foo"];
    const text = "foo";
    expect(tokenHighlighter.highlight(text, tokens)).toEqual(
      tokenHighlighter._wrapText(text)
    );
  });
  ...
  it("should highlight multiple matches for multiple tokens", () => {
    ...
    const expectedText =
      "foo " +
      tokenHighlighter._wrapText("bar") +
      " " +
      tokenHighlighter._wrapText("baz") +
      " foo";
  ...
  it("should highlight tokens within tokens", () => {
    ...
    const expectedText =
      "bar " +
      tokenHighlighter._wrapText(tokenHighlighter._wrapText("foo") + "bar") +
      " baz";
    expect(tokenHighlighter.highlight(text, tokens)).toEqual(expectedText);
  });
  it("should highlight using sanitized text", () => {
    ...
    const expectedText =
      tokenHighlighter._wrapText("Foo") +
      " " +
      tokenHighlighter._wrapText("bar") +
      " baz";
    expect(tokenHighlighter.highlight(text, tokens)).toEqual(expectedText);
