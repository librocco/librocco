...
describe('SimpleTokenizer', () => {
  ...
  it('should handle cyrillic', () => {
    const tokenizer = new SimpleTokenizer();
    expect(tokenizer.tokenize('Есть хоть одна девушка, которую ты хочешь? Или ты устал от женщин')).toEqual(['Есть', 'хоть', 'одна', 'девушка', 'которую', 'ты', 'хочешь', 'Или', 'ты', 'устал', 'от', 'женщин']);
  });
});