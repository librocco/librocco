...
describe("Search", () => {
  ...
  describe("TF-IDF", () => {
    ...
    it("should compute for multiple tokens within the corpus", () => {
      assertTfIdf(
        ["document", "node"],
        documents[3],
        calculateTfIdf(4, 1) + calculateTfIdf(3, 2)
      );
      assertTfIdf(
        ["ruby", "and"],
        documents[2],
        calculateTfIdf(2, 1) + calculateTfIdf(1, 1)
      );
    });
    it("should compute for tokens that are not within the corpus", () => {
  ...
  describe("search", () => {
    ...
    it("should give documents containing words with a lower IDF a higher relative ranking", () => {
      ...
      for (let i = 0; i < 10; i++) {
        addDocument(
          "foo foo baz foo foo baz foo foo baz foo foo baz foo foo baz foo foo baz foo foo baz foo foo"
        );
      }
